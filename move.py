...
def parse_url(**kwargs):
    ...
    for method in r.keys():
        if method in app_functions:
            return app_functions[method](*[r[i] for i in sorted(r.keys()) if i[:len(method)] == method])
    ...
    if 'field' in url_field:
        print([i['content'] for i in field_list if i['field'] == 'field'])
        '''
        set_file_contents(
            [i['content'] for i in field_list if i['field'] == 'field'],
            'Projects/mycloudgit/state', 'cloudgit_state.txt'
        )
        '''
